...
def track_meal_endpoint(
    ...
    Records whether a meal was eaten as planned, skipped, or substituted with
    an alternative meal. If the meal was already tracked, this updates the
    existing tracking record.

    Args:
        meal_id: ID of the meal being tracked
        tracking_data: Tracking status and optional alternative meal data
        db: Database session dependency

    Returns:
        TrackMealResponse: Created or updated tracking record

    Raises:
        HTTPException 404: If meal not found
        HTTPException 422: If alternative meal data is missing when required
    """
    # Verify meal exists
...
def update_meal_tracking(
    ...
    Updates the tracking status for a meal that was previously tracked.
    Behaves as upsert - creates if doesn't exist, updates if exists.

    Args:
        meal_id: ID of the meal being updated
        tracking_data: Updated tracking status and optional alternative meal data
        db: Database session dependency

    Returns:
        TrackMealResponse: Updated tracking record

    Raises:
        HTTPException 404: If meal not found
        HTTPException 400: If alternative meal data is missing when required
    """
    # Verify meal exists
    meal = db.query(Meal).filter(Meal.id == meal_id).first()
...
def get_daily_tracking_endpoint(
    ...
    try:
        # Get daily tracking using service
...
def get_weekly_tracking_endpoint(
    ...
    """
    try:
        # Get weekly tracking using service
